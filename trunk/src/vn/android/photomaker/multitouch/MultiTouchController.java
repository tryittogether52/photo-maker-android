 ...
 * Usage:
 *   <code>
 *   public class MyMTView extends View implements
 *          MultiTouchObjectCanvas<PinchWidgetType> {
 *
 *       private MultiTouchController<PinchWidgetType>
 *              multiTouchController = new MultiTouchController<PinchWidgetType>(this);
 *
 ...
 *       }
 *
 *       // ... then implement the MultiTouchObjectCanvas interface here,
 *       // see details in the comments of that interface.
 *   }
 *   </code>
 *
 * Changelog:
 *   2010-06-09 v1.5.1  Some API changes to make it possible to
 *                      selectively update or not update scale / rotation.
 *                      Fixed anisotropic zoom.  Cleaned up rotation code.
 *                      Added more comments.  Better var names. (LH)
 *   2010-06-09 v1.4    Added ability to track pinch rotation (Mickael
 *                      Despesse, author of "Face Frenzy") and anisotropic pinch-zoom (LH)
 *   2010-06-09 v1.3.3  Bugfixes for Android-2.1; added optional debug info (LH)
 *   2010-06-09 v1.3    Ported to Android-2.2 (handle ACTION_POINTER_* actions);
 *                      fixed several bugs; refactoring; documentation (LH)
 *   2010-05-17 v1.2.1  Dual-licensed under Apache and GPL licenses
 *   2010-02-18 v1.2    Support for compilation under Android 1.5/1.6 using
 *                      introspection (mmin, author of handyCalc)
 *   2010-01-08 v1.1.1  Bugfixes to Cyanogen's patch that only showed up in more
 *                      complex uses of controller (LH)
 ...
 *
 * Known usages:
 * - Mickael Despesse's "Face Frenzy" face distortion app,
 *   to be published to the Market soon
 ...
 * - Formerly: The browser in cyanogenmod (and before that, JesusFreke),
 *   and other firmwares like dwang5.  This usage has been
...
public class MultiTouchController<T> {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        try {
            ...
            if (DEBUG)
                Log.i("MultiTouch", "Got here 1 - " +
                        multiTouchSupported + " " + mMode + " " +
                        handleSingleTouchEvents + " " + pointerCount);
    ...
    private void decodeTouchEvent(int pointerCount, float[] x, float[] y,
            ...
            long eventTime) {
        if (DEBUG)
            Log.i("MultiTouch", "Got here 5 - " + pointerCount +
                    " " + action + " " + down);
    ...
    private void performDragOrPinch() {
        ...
        if (!mDragOccurred) {
            if (!pastThreshold(Math.abs(deltaX), Math.abs(deltaY), newScale)) {
                if (DEBUG) {
                    Log.i("MultiTouch", "Change received by performDragOrPinch " +
    ...
    private void multiTouchController() {
        if (DEBUG)
            Log.i("MultiTouch", "Got here 6 - " + mMode + " " +
        ...
        if (DEBUG)
            Log.i("MultiTouch", "Got here 7 - " + mMode + " " +
                    mCurrPt.getNumTouchPoints() + " " + mCurrPt.isDown() +
                    mCurrPt.isMultiTouch());
    }
    ...
    // ---------------------------------------------------------------------------
        ...
        private void set(int numPoints, float[] x, float[] y,
            ...
            if (DEBUG)
                Log.i("MultiTouch", "Got here 8 - " + +numPoints + " " +
                        x[0] + " " + y[0] + " " + (numPoints > 1 ? x[1] : x[0]) + " "
